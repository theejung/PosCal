...
from transformers import (WEIGHTS_NAME, BertConfig, BertTokenizer, AdamW, get_linear_schedule_with_warmup) #WarmupLinearSchedule
...
def load_and_cache_examples(args, task, tokenizer, data_type='train', show_stats=True):
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        data_dir_eval = args.eval_dataset if args.eval_dataset is not None else args.data_dir
...
def train(args, train_dataset, model, tokenizer, num_labels, valid_dataset=None):
    ...
    for epoch in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            ##Update p_emps
            if (num_epochs > args.calloss_start_epochs and args.poscal_train and ((step+1) % update_freq == 0)) \
            ...
            if valid_dataset is not None:
                ...
                if num_epochs > 0 and global_step % 500 == 0:
                    for val_step, val_batch in enumerate(tqdm(valid_dataloader, desc="Valid")):
                    ...
                    if len(val_loss_per_steps) > 10 and np.mean(val_loss_per_steps[-10:]) < val_loss:
                        break
                    else:
        ...
        with open(output_train_file, 'a') as writer:
            ...
            writer.write("%.4f\t%.4f\t%.4f\t" %(epoch_loss, epoch_loss_mle, epoch_loss_cal))
            writer.write("%.4f" %(train_ece))
            writer.write("\n")

        if valid_dataset is not None:
            if num_epochs > 1:
                if len(val_loss_per_steps) > 5 and np.mean(val_loss_per_steps[-5:]) < val_loss:
                    print("Early stopping in num_epochs %d, steps %d" %(num_epochs, step+1))
                    train_iterator.close()
                    break
...
def evaluate(args, model, tokenizer, num_labels,prefix=""):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        eval_ece = np.sum(freq_p_hats*(abs(p_confs-p_emps)))/np.sum(freq_p_hats[:,0])
...
def main():
    ...
    if args.do_train:
        train_dataset = load_and_cache_examples(args, args.task_name,
                tokenizer, data_type='train')
        valid_dataset = load_and_cache_examples(args, args.task_name,
                tokenizer, data_type='dev')

        if not os.path.exists(os.path.join(args.output_dir,'training_args.bin')):
            global_step, tr_loss = train(args, train_dataset, model, tokenizer,
                     num_labels, valid_dataset=valid_dataset)
